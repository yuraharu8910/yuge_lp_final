(function () {
  ...
  /* ══════════════════════════════════════
     ① FV スライドショー
     一定時間ごとに .active クラスを次のスライドに移す
  ══════════════════════════════════════ */
  (function () {
    // スライド要素とドット要素をすべて取得
    var slides = document.querySelectorAll('.fv-slide');
    var dots   = document.querySelectorAll('.fv-dot');

    // スライドが1枚以下なら何もしない
    if (slides.length < 2) return;

    var currentIndex = 0;  // 現在表示中のスライド番号（0始まり）
    var INTERVAL = 5000;   // 5秒ごとに切り替え（ミリ秒単位）
  ...
  /* ══════════════════════════════════════════════════
     ③ ブラウザ「戻る」ボタンによる外部離脱を防ぐ

     【仕組みの説明】
     history.pushState() = ブラウザの「履歴スタック（History Stack）」に
     ページを追加するメソッド。実際にはページ遷移しないが、
     「戻る」ボタンを押したときに popstate イベントが発生するようになる。

     通常、外部リンクをクリックして戻ると前の外部サイトに戻ってしまうが、
     このページの履歴を1件追加しておくことで、
     「戻る」が押されたときにこのLP自体に留まることができる。
  ══════════════════════════════════════════════════ */
  (function () {
    // ページロード時に「このページの状態」を履歴に1件追加する
    // 第1引数：状態オブジェクト（ここでは {page:'top'} という識別情報）
    // 第2引数：タイトル（多くのブラウザでは無視されるが慣例的に渡す）
    // 第3引数：URLは変えない（'' = 現在のURLのまま）
    if (window.history && window.history.pushState) {
      history.pushState({ page: 'top' }, document.title, '');

      // popstate イベント = 「戻る」「進む」ボタンが押されたときに発火する
      window.addEventListener('popstate', function (e) {
        // 戻ろうとしたら、もう一度このページを履歴に積み直す
        // これにより「戻る」を押すたびにこのページに留まる
        history.pushState({ page: 'top' }, document.title, '');

        // ▼ 任意：「戻る」が押されたときの追加アクション
        // 例えばFVまでスムーズスクロールで戻すと自然な体験になる
